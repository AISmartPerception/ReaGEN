...
class Stage:
    ...
    def get_user_text(self, sample: dict, W: int, H: int, dataset_name: str):
        ...
        if dataset_name == "MMMU_Pro":
            full_prompt, labels, gt = format_mmmu_sample(sample)  # e.g., question + A)-J) options
    ...
    def render_output(self, parsed_json: dict, W: int = None, H: int = None, logger = None):
        ...
        #     return f"Raw output: {text}"
        if parsed_json is None:
            if self.name == "BBOX":
                return f"Bounding Box: [0, 0, {W}, {H}]", (0, 0, W, H)
            # elif self.name == "TEXT.DETECTION":
            #     return "No text detected"
            # elif self.name == "COLOR.ATTRIBUTE":
            #     return "Object: unknown, Color: null"
            # elif self.name == "SPATIAL.RELATION":
            #     return "Spatial Relation: none"
            # elif self.name == "COUNT":
            #     return "Count: 0"
            elif self.name == "FINAL":
                return "Answer: unknown, Evidence IDs: [1]"
            # else:
            #     return f"Raw output: {parsed_json}"
        elif parsed_json == "Invalid JSON format":
            return f"Invalid JSON format", (0, 0, W, H)
        ...
        try:
            ...
            # if self.name == "BBOX":
            #     try:
            #         bbox = parsed_json.get("bbox", [])
            #         if len(bbox) == 4:
            #             x1, y1, x2, y2 = bbox
            #             box = BBox(int(x1), int(y1), int(x2), int(y2), conf=None)
            #             box = _pad_box(box, W, H, logger=logger)
            #             return f"Bounding Box: [{box.x1}, {box.y1}, {box.x2}, {box.y2}]", (box.x1, box.y1, box.x2, box.y2)
            #         else:
            #             # return f"Invalid bbox format: {bbox}"
            #             if logger:
            #                 logger.warning(f"Invalid bbox format: {bbox}")
            #             return f"BBOX: [0, 0, {W}, {H}]", (0, 0, W, H)
            #     except Exception as e:
            #         if logger: 
            #             logger.warning(f"BBOX parse failed: {e}")
            #         # return f"BBOX parsing failed: {parsed_json}"
            #         return f"Bounding Box: [0, 0, {W}, {H}]", (0, 0, W, H)
            ...
            # else:
            #     return f"Unknown stage '{self.name}': {text}"
        except Exception as e:
            if logger: 
                logger.warning(f"{self.name} parse failed: {e}")

            if self.name == "BBOX":
                return f"{self.name} parsing failed: {parsed_json}", (0, 0, W, H)
            else:
    ...
    def get_iq_embeddings(self, inputs, question_inputs):
        ...
        # aligned_vision_embeds = (
        #     vision_out.last_hidden_state if hasattr(vision_out, "last_hidden_state") else vision_out
        # ).unsqueeze(0)
        aligned_vision_embeds = vision_out[0].unsqueeze(0)

        # image_emb = aligned_vision_embeds.unsqueeze(0)  # [1, num_patches, emb_dim]
        token_counts = []
    ...
    def run(self, sample_id, sample: Dict[str, Any], blackboard_text: str, images: list, mem_names: list, logger = None):
        ...
        # inputs = self.processor(text=[chat], images=images, return_tensors="pt").to(self.model.device)
        ...
        with torch.inference_mode():
            ...
            # mean_attention_mass = attn_mass_for_spans(gen.attentions, mem_spans, inputs["input_ids"].shape[1], self.processor, input_ids, logger=logger)
            if self.name != "DIRECT_ANSWER":
            ...
            if hasattr(gen, 'scores') and gen.scores:
                # gen.scores is a tuple of tensors, each of shape (batch_size, vocab_size)
                # Convert to tensor of shape (num_steps, vocab_size) for batch_size=1
                logits_sequence = torch.stack(gen.scores).squeeze(1)  # Remove batch dimension
                uncertainty_score = compute_stage_uncertainty(logits_sequence)
        ...
        #     if parsed_json == "Invalid JSON format":
        #         return {
        #             "raw_text": text,
        ...
        #     rendered_output, coords = self.render_output(parsed_json, W=W, H=H, logger=logger)
