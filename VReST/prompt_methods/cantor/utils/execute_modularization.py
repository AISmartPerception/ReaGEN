...
def execute_modularization(query, img_path, model, decision):
    ...
    for prompt in prompts:
        ...
        response = model.get_response(prompt, img_path)['response']
        # try:
        #     response.resolve()
        # except:
        ...
        try:
            # print(response.text)
            decision["sub-answers"].append(response) 
        except:
            decision["sub-answers"].append(" ") 
...
def summary(query, img_path, model, decision):
    # with open(args.decision_path, 'r') as file:
    #     decision = json.load(file)
